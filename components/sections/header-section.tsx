...
export function HeaderSection() {
  ...
  return (
    <header 
      className={`${
        isScrolled 
          ? 'bg-emerald-900/95 backdrop-blur-sm shadow-md' 
          : 'bg-emerald-900'
    ...
    >
      ...
      {/* Mobile Menu */}
      <div
        className={`fixed inset-0 bg-emerald-900/95 backdrop-blur-sm z-40 md:hidden transition-all duration-300 ${
...
function NavItem({ 
  ...
  return (
    <div className="relative group">
      <Link 
        href={href} 
        className="text-gray-300 hover:text-white flex items-center group-hover:text-white transition-colors duration-200"
      >
        {label}
        {hasDropdown && (
          <svg 
...
function MobileNavItem({ href, label }: { href: string; label: string }) {
  return (
    <Link 
      href={href} 
      className="text-white text-xl font-medium hover:text-emerald-300 transition-colors duration-200"
